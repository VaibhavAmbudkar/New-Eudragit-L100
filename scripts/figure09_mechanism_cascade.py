...
def dots(cx, cy, n, color, spread, seed, marker='o', size=45, ymax=6.5):
    np.random.seed(seed)
    placed=0; tries=0
    while placed < n and tries < n*30:
        dx,dy=np.random.uniform(-spread,spread,2); tries+=1
        if dy > ymax: continue          # keep clear of the label band at the panel top
        if marker=='o':
...
ax.text(cx,cy+Ry-1.2,"pH \u2191",ha='center',va='top',fontsize=21,fontweight='bold',color="#922B21")
...
ax.text(cx,cy+Ry-1.2,"COO\u207b",ha='center',va='top',fontsize=20,fontweight='bold',color=PURPLE)

cx,cy=center(2); panel_bg(cx,cy,bgs[2]); chains(cx,cy,2.4,BLUE,3,6); dots(cx,cy,10,PURPLE,6.5,7)
for ang in range(0,360,60):
    a=np.radians(ang)
    ax.annotate('',xy=(cx+6.8*np.cos(a),cy+6.8*np.sin(a)),xytext=(cx+3.2*np.cos(a),cy+3.2*np.sin(a)),arrowprops=dict(arrowstyle='-|>',color=RED,lw=2.0))
ax.text(cx,cy+Ry-1.2,"repulsion",ha='center',va='top',fontsize=18,fontweight='bold',color=RED)

cx,cy=center(3); panel_bg(cx,cy,bgs[3]); chains(cx,cy,3.4,ORANGE,4,8); dots(cx,cy,45,WATER,9.5,9); dots(cx,cy,8,PURPLE,9,10)
ax.text(cx,cy+Ry-1.2,"swollen",ha='center',va='top',fontsize=19,fontweight='bold',color="#1A5276")

cx,cy=center(4); panel_bg(cx,cy,bgs[4]); chains(cx,cy,3.4,ORANGE,4,11)
np.random.seed(12)
for _ in range(7):
    tx,ty=cx+np.random.uniform(-7,7), cy+np.random.uniform(-7,5.0)
    ax.add_patch(Circle((tx,ty),0.85,facecolor=GREEN,edgecolor='white',lw=0.7,zorder=6))
    a=np.random.uniform(0,2*np.pi)
    ax.annotate('',xy=(tx+2.2*np.cos(a),ty+2.2*np.sin(a)),xytext=(tx,ty),arrowprops=dict(arrowstyle='-|>',color=GREEN,lw=1.7))
ax.text(cx,cy+Ry-1.2,"TEC mobile",ha='center',va='top',fontsize=18,fontweight='bold',color=GREEN)

cx,cy=center(5); panel_bg(cx,cy,bgs[5]); chains(cx,cy,3.6,ORANGE,3,13); dots(cx,cy,30,WATER,10,14)
np.random.seed(15)
for _ in range(9):
    a=np.random.uniform(0,2*np.pi); r=np.random.uniform(2,5.5); dx,dy=r*np.cos(a),r*np.sin(a)
    ax.add_patch(Circle((cx+dx,cy+dy),0.7,facecolor=DRUG,edgecolor='white',lw=0.6,zorder=6))
    ax.annotate('',xy=(cx+dx*1.25,cy+dy*1.25),xytext=(cx+dx,cy+dy),arrowprops=dict(arrowstyle='-|>',color=DRUG,lw=1.4))
ax.text(cx,cy+Ry-1.2,"release",ha='center',va='top',fontsize=19,fontweight='bold',color=DRUG)

# titles + notes
for i in range(6):
    cx,cy=center(i)
    ax.text(cx, cy-Ry-2.6, titles[i], ha='center', va='top', fontsize=22, fontweight='bold')
    ax.add_patch(FancyBboxPatch((cx-R, cy-Ry-16.8), 2*R, 9.4, boxstyle="round,pad=0.2",
                 facecolor='#F7F7F7', edgecolor='#DDDDDD', lw=1, zorder=1))
    ax.text(cx, cy-Ry-12.1, notes[i], ha='center', va='center', fontsize=16.5, linespacing=1.45)
...
ax.legend(handles=leg, loc='upper center', ncol=6, fontsize=15, frameon=True,
          framealpha=0.96, edgecolor='#CCCCCC', bbox_to_anchor=(0.5, 0.087))
